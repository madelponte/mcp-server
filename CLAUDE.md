...
The Agentic Web Search tool depends on external services pointed at by `WEB_SEARCH_*` config: **SearXNG** (search backend — JSON output must be enabled, which the bundled `searxng/settings.yml` does), **FlareSolverr** (automatic local fallback for bot-walled and JavaScript-empty pages), **Firecrawl** (optional final API fallback, enabled by `WEB_SEARCH_FIRECRAWL_API_KEY`), and **Apache Tika** (text extraction for PDF/Office/OpenDocument/RTF/EPUB). `docker-compose.yml` starts the three local services; Firecrawl is called through its configured API URL. The non-web tool groups have no local-service dependencies.

`fetch_page` has a fallback ladder for failures that leave it without usable content: direct HTTP plus BeautifulSoup extraction first, FlareSolverr second, and Firecrawl last. `_is_contentless` detects a JavaScript shell separately from a bot wall; network failures, gone/unavailable statuses (404/410/451), detected walls/rate limits, and empty shells finish this ordered ladder before the tool raises. Other ordinary HTTP errors still fail immediately so they do not consume Firecrawl credits needlessly. Firecrawl requests rendered HTML from the v2 scrape endpoint, then sends that HTML through the same BeautifulSoup extraction paths as local responses so text, structured, section, and query modes stay consistent. `search_web` enrichment deliberately remains direct-only so a handful of difficult results cannot consume browser time or Firecrawl credits.
