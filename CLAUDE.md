...
`fetch_page` begins with a streamed direct probe. Documents, JSON/XML, plain text, and unsupported binary media finish on that lightweight direct path; HTML closes the probe without downloading the body and goes to FlareSolverr first. A generic structural/content quality gate accepts clear page content and sends blocked or unusable renders to Firecrawl. Ambiguous pages use the optional classifier; without a configured classifier that stage is skipped. Hedged Firecrawl is available but disabled by default, while repeated failures for distinct URLs on one host open a short-lived circuit that bypasses FlareSolverr when Firecrawl is configured. Firecrawl and FlareSolverr HTML use the same BeautifulSoup extraction paths so text, structured, section, and query modes stay consistent. `search_web` enrichment deliberately remains direct-only and cannot use browser/Firecrawl fallbacks.
